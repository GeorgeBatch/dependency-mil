...
from source.data.datamodule_detailed import LungSubtypingDM
from source.losses import custom_cross_entropy_loss
from source.metrics import multi_label_roc
from source.feature_aggregation.combined_model import get_model
...
def train(
    ...
    for batch_index, batch in enumerate(train_dataloader):
        ...
        # noqa: F841 - will use bag_patch_locs in the future
        bag_feats, bag_label, label_weight_mask, bag_lens, bag_patch_locs, patch_status, patch_status_weight_mask = batch

        # ipdb.set_trace()

        # bag_feats = dropout_patches(feats, config.dropout_patch)
        ...
        #   bag_prediction
        #   attention map: A
        #   attention-reweighted bag features: B
        #   aux (dsmil: max_ins_prediction, abmil: None)
        #   instance_logits: instance-level logits for each patch (if asked for, else None)
        bag_prediction, A, B, aux, instance_logits = milnet(bag_feats, bag_lens)  # bag_prediction: (1, num_classes)

        # import ipdb; ipdb.set_trace()

        # compute loss
        bag_loss = criterion(inputs_list=[bag_prediction, aux], targets=bag_label, weights=label_weight_mask)
        if instance_logits is not None:
            instance_loss = criterion(inputs_list=[instance_logits], targets=patch_status, weights=patch_status_weight_mask)
            loss = (
                (1 - config.instance_loss_coef) * bag_loss
                + config.instance_loss_coef * instance_loss
            )
        else:
            loss = bag_loss
...
def evaluate(
    ...
    with torch.no_grad():
        for batch_index, batch in enumerate(eval_dataloader):
            ...
            #   bag_prediction,
            #   attention map: A,
            #   attention-reweighed bag features: B,
            #   aux (dsmil: max_ins_prediction, abmil: None)
            #   instance_logits: instance-level logits for each patch (if asked for, else None)
            bag_prediction, A, B, aux, instance_logits = milnet(
                bag_feats, bag_lens
            )  # bag_prediction: (1, num_classes)
            ...
            if config.average_instance_and_bag_pred and config.architecture_name == 'dsmil':
                test_predictions.extend(
                    [(0.5 * torch.sigmoid(aux) + 0.5 * torch.sigmoid(bag_prediction)).cpu().numpy()])
            else:
...
def main():
    ...
    data_module = LungSubtypingDM(config=config, device=torch_device)
    data_module.setup()
    print("Data module setup complete.\n")

    milnet = get_model(
        instance_embedder_name=config.instance_embedder_name,
        instance_classifier_name=config.instance_classifier_name,
        bag_aggregator_name=config.architecture_name,
        aggregator_kwargs=config.aggregator_kwargs,
        class_connector_name=config.class_connector_name,
        classifier_name=config.classifier_name,
        num_classes=config.num_classes,
        feats_size=config.feats_size,
        instance_embedder_output_size=config.instance_embedder_output_size,
    )
    milnet = milnet.to(device=torch_device)
    ...
    if os.path.isfile(config.saved_weights_pth):
        ...
        milnet.load_state_dict(torch.load(config.saved_weights_pth), strict=True)
    ...
    if 'train' in config.train_test_mode:
        ...
        optimizer = optimizer_class(milnet.parameters(), **config.optimizer_kwargs)
        ...
        save_path = os.path.join(save_dir_path, f'run-{total_runs + 1}-experiment-{config.experiment_name}-wandb-{wandb.run.id}.pth')
        ...
        for epoch in range(1, config.num_epochs + 1):
            ...
            train_loss_bag = train(train_dataloader=train_dataloader, milnet=milnet, criterion=criterion,
            ...
            valid_loss_bag, avg_score, aucs, thresholds_optimal, valid_logs_dict = evaluate(
                eval_dataloader=valid_dataloader, milnet=milnet, criterion=criterion, mode='valid',
            ...
            if current_score >= best_score:
                ...
                torch.save(milnet.state_dict(), save_path)
    ...
    if 'test' in config.train_test_mode:
        ...
        if 'train' in config.train_test_mode:
            ...
            milnet.load_state_dict(torch.load(save_path))
        ...
        test_loss_bag, avg_score, aucs, _, test_logs_dict = evaluate(eval_dataloader=test_dataloader, milnet=milnet,
