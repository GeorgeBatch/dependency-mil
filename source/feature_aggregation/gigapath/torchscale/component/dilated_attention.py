...
class DilatedAttention(MultiheadAttention):
    ...
    def scattering(self, outs, lses, seq_len, bsz, offset=0):
        ...
        # sum_attn_weights = torch.stack(all_lses, dim=0).sum(0)
        # return out
        return out, all_lses
    ...
    def forward(
        ...
        attn, _ = self.scattering(outs, lses, tgt_len, bsz, offset=offset)
        # attn = self.scattering(outs, lses, tgt_len, bsz, offset=offset)
        # attn_weights = []
        # for lse in all_lses:
        #     lse = lse.squeeze()
        #     lse = rearrange(lse, '(b h) l -> b h l', h=self.num_heads)
        #     lse = lse[:, :, 1:] # exclude [CLS] token
        #     attn_weights.append(lse)
        ...
        # print(attn.shape)
        act_weights = attn.mean(dim=-1)

        return attn, act_weights
