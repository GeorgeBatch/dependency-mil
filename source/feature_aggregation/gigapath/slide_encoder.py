...
class LongNetViT(nn.Module):
    ...
    def forward(self, x, coords, all_layer_embed=False, access_attn=False):
        """
        ...
        if all_layer_embed:
            out_dict = self.encoder(src_tokens=None, token_embeddings=x, return_all_hiddens=all_layer_embed)
            # x_list = self.encoder(src_tokens=None, token_embeddings=x, return_all_hiddens=all_layer_embed)["encoder_states"]
            x_list = out_dict["encoder_states"]
        else:
            out_dict = self.encoder(src_tokens=None, token_embeddings=x)
            # x_list = [self.encoder(src_tokens=None, token_embeddings=x)["encoder_out"]]
            x_list = [out_dict["encoder_out"]]
        # load a list of attention values for each layer
        attn_layers_outs = out_dict["attn_layers_weights"] if access_attn else None
        ...
        return outcomes, attn_layers_outs
...
def create_model(
    ...
    local_dir: str = os.path.join(
        os.path.expanduser("~"),
        ".cache/huggingface/hub/models--prov-gigapath--prov-gigapath/blobs/",  # change from original: ".cache/"
    ),
    **kwargs,
):
